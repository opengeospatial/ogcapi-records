...
For `"ids"`, on the other hand, `"?ids=,&"` is valid.  It is a list of two null or empty identifiers which is a valid request.  In this case the response would be an empty result set since a record cannot have a null or empty identifier.
