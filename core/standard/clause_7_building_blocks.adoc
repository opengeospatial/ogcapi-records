...
The `q` parameter is a comma-separated list of search terms.  If any of these terms appear in one or more searched text field of a record then that record can be included in the result set (logical OR).  If any search term specified using the `q` parameter contains one or more whitespaces, this implies that all of the specified search terms must appear in one or more searched text field of a record before that record can be included in the result set (logical AND).
