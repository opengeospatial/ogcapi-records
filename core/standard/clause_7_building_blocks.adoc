...
This Building Blocks clause specifies the requirements classes that define building blocks that can be used to define various <<catalog-deployments,catalog deployments>>.
...
|conformsTo |optional |The extensions/conformance classes used in this record. |`properties.conformsTo`
|===
...
|geometry |optional |A geometry associated with the resource that is used for discovery.  Can be null if there is no associated geometry. |`geometry`
...
|links |**required** |A list of links related to this record.|`links`
|===
...
|_**extent**_ |optional |The spatiotemporal coverage of this collection.
|_**itemType**_ |**required** |Indicates the type of items in this collection. Fixed to "record".
|_**crs**_ |optional |A list of coordinate reference systems used for spatiotemporal values.
|type |**required** |The nature or genre of this collection. Fixed to "catalog".
...
|recordsArrayName |optional |The name of the array member in the catalog used to encode records in-line.  The default value is `records`.
|records |optional |An array of records encoded in-line in the catalog.
|===

NOTE: The properties _id_, _title_, _description_, _links_, _extent_, _itemsType_ and _crs_ were inherited from http://docs.opengeospatial.org/is/17-069r3/17-069r3.html#\_collection\_[OGC API - Features - Part 1: Core].
...
This examples below show that by default, response records are sorted by the `updated` sortable and the `area` sortable with the most recent record with the largest area being presented first.
...
The use of keywords from a controlled vocabulary is encoded in a record using the <<sc_keywords_and_themes,`themes`>> member.  Consider a record that have the following `themes` member:
...
The purpose of autodiscovery is, knowing the location of a web page, to find the addresses(s) of that page's associated catalog(s).  For example, a client could retrieve the landing page of an OGC API deployment, find the location of the site's searchable catalog by locating the `rel=http://www.opengis.net/def/rel/ogc/1.0/catalog` link in the landing page and then using that catalog to search for resources offered by the site.

==== Autodiscovery links

===== Discovering searchable catalogs

include::requirements/autodiscovery/REQ_autodiscovery_searchable_link.adoc[]

===== Discovering crawlable catalogs

include::requirements/autodiscovery/REQ_autodiscovery_crawlable_link.adoc[]

===== Discovering local resources catalogs

include::requirements/autodiscovery/REQ_autodiscovery_local-resources_link.adoc[]
...
.link:https://github.com/opengeospatial/ogcapi-records/blob/master/core/openapi/schemas/recordCommonProperties.yaml[Schema the set of common record properties (recordCommonProperties.yaml)]
[source,YAML]
----
include::../openapi/schemas/recordCommonProperties.yaml[]
----

NOTE: Referenced schemas can be found at https://github.com/opengeospatial/ogcapi-records/blob/master/core/openapi/schemas/common/recordCommonProperties.yaml[recordCommonProperties.yaml], https://github.com/opengeospatial/ogcapi-records/blob/master/core/openapi/schemas/time.yaml[time.yaml], https://schemas.opengis.net/ogcapi/features/part1/1.0/openapi/schemas/geometryGeoJSON.yaml[geometryGeoJSON.yaml], https://github.com/opengeospatial/ogcapi-records/blob/master/core/openapi/schemas/language.yaml[language.yaml], https://github.com/opengeospatial/ogcapi-records/blob/master/core/openapi/schemas/theme.yaml[theme.yaml], https://github.com/opengeospatial/ogcapi-records/blob/master/core/openapi/schemas/contact.yaml[contact.yaml], https://github.com/opengeospatial/ogcapi-records/blob/master/core/openapi/schemas/license.yaml[license.yaml], https://github.com/opengeospatial/ogcapi-records/blob/master/core/openapi/schemas/common/linkBase.yaml[common/linkBase.yaml].
...
.link:https://github.com/opengeospatial/ogcapi-records/blob/master/core/openapi/schemas/catalog.yaml[Schema for a catalog in JSON(catalog.yaml)]
[source,YAML]
----
include::../openapi/schemas/catalog.yaml[]
----

NOTE: Referenced schemas can be found at https://schemas.opengis.net/ogcapi/features/part1/1.0/openapi/schemas/collection.yaml[collection.yaml], https://github.com/opengeospatial/ogcapi-records/blob/master/core/openapi/schemas/language.yaml[language.yaml], https://github.com/opengeospatial/ogcapi-records/blob/master/core/openapi/schemas/theme.yaml[theme.yaml], https://github.com/opengeospatial/ogcapi-records/blob/master/core/openapi/schemas/contact.yaml[contact.yaml], https://github.com/opengeospatial/ogcapi-records/blob/master/core/openapi/schemas/license.yaml[license.yaml].
...
[[clause-oas30]] 
=== Requirements Class "OpenAPI 3.0" (Building Block)
