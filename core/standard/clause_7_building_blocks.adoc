...
Although this document does not mandate any particular encoding for a record collection, two define conformance classes are defined:
...
The schema for links added to the <<query-response,links>> and <<query-response,linkTemplates>> sections is based on https://tools.ietf.org/html/rfc8288[RFC 8288] and is described in https://schemas.opengis.net/ogcapi/features/part1/1.0/openapi/schemas/link.yaml[OGC API - Features - Part 1: Core].
...
Servers conforming to this requirments class define their API using an 
