...
In the case where all the records of a catalog use the same set of extensions, and to prevent unnecessary duplication, there is also a `conformsTo` property at the https://schemas.opengis.net/ogcapi/records/part1/1.0/openapi/schemas/catalog.yaml[catalog level] that may be used to declare which extensions are used in all records of the catalog.  
...
The https://schemas.opengis.net/ogcapi/features/part1/1.0/openapi/schemas/link.yaml[link object] and the https://schemas.opengis.net/ogcapi/records/part1/1.0/openapi/schemas/linkTemplate.yaml[templated link object] are based on https://datatracker.ietf.org/doc/html/rfc8288[RFC 8288 (Web Linking)] which includes a `hreflang` attribute that can be used to state the language of a referenced resource.  This can be used to include links to the same record in different languages. 
...
* An additional query parameter (for example, https://schemas.opengis.net/ogcapi/records/part1/1.0/openapi/parameters/language.yaml["language"] or "l") that overrides the Accept-Language header of the HTTP request.
======================================================================
...
.Example of Links and Link-Templates in a Record
[source,json]
----
include::../examples/json/associations.json[]
----
...
NOTE: Care should be exercised when there is a large number of links that need to be presented.  In such cases, it would be prudent to avoid encoding all the links in the HTTP header and instead including them in the body of the response in the link and/or link templates sections.
...
|resourceLanguages |optional |The list of languages in which records from this catalog can be represented.
...
NOTE: The properties _id_, _itemType_, _title_, _description_, _extent_, _crs_ and _links_ are inherited from http://docs.opengeospatial.org/is/17-069r3/17-069r3.html#\_collection\_[OGC API - Features - Part 1: Core].
...
Similarly, a <<clause-local-resources-catalog,local resources catalog>> at the `/processes` endpoint uses an array named `processes` to encode the records of the catalog and so the value of the `recordsArrayName` property would be:
...
[source,json]
----
include::../examples/json/records.json[]
----
...
[[schema_sortables]]
.link:https://schemas.opengis.net/ogcapi/records/part1/1.0/openapi/responses/Sortables.yaml[Schema for the list of sortable properties (Sortables.yaml)]
[source,YAML]
----
...
include::requirements/building-blocks/json/REQ_record-content-profile.adoc[]
...
include::recommendations/building-blocks/json/REC_header-profile-link.adoc[]

[[schema_json_record]]
.link:https://schemas.opengis.net/ogcapi/records/part1/1.0/openapi/schemas/recordGeoJSON.yaml[Schema for a catalog record encoded as GeoJSON (recordGeoJSON.yaml)]
[source,YAML]
----
include::../openapi/schemas/recordGeoJSON.yaml[]
----

.link:https://schemas.opengis.net/ogcapi/records/part1/1.0/openapi/schemas/recordCommonProperties.yaml[Common record properties schema (recordCommonProperties.yaml)]
[source,YAML]
----
...
* https://schemas.opengis.net/ogcapi/records/part1/1.0/openapi/schemas/time.yaml[time.yaml],
* https://schemas.opengis.net/ogcapi/features/part1/1.0/openapi/schemas/geometryGeoJSON.yaml[geometryGeoJSON.yaml],
* https://schemas.opengis.net/ogcapi/records/part1/1.0/openapi/schemas/language.yaml[language.yaml],
* https://schemas.opengis.net/ogcapi/records/part1/1.0/openapi/schemas/theme.yaml[theme.yaml],
* https://schemas.opengis.net/ogcapi/records/part1/1.0/openapi/schemas/format.yaml[format.yaml],
* https://schemas.opengis.net/ogcapi/records/part1/1.0/openapi/schemas/contact.yaml[contact.yaml],
* https://schemas.opengis.net/ogcapi/records/part1/1.0/openapi/schemas/license.yaml[license.yaml],
* https://schemas.opengis.net/ogcapi/records/part1/1.0/openapi/schemas/common/linkBase.yaml[linkBase.yaml].
======================================================================
...
[NOTE]
=================================================
This standard does not define a normative schema for a record encoding that is not GeoJSON.  However, this https://schemas.opengis.net/ogcapi/records/part1/1.0/openapi/schemas/recordJSON.yaml[informative schema] is offered as an example of what a plain JSON record schema might look like.
=================================================
...
include::requirements/building-blocks/json/REQ_catalog-content-profile.adoc[]

See also recommendation: <<ec_json_header-profile-link,/rec/json/header-profile-link>>.

[[schema_json_collection]]
.link:https://schemas.opengis.net/ogcapi/records/part1/1.0/openapi/schemas/catalog.yaml[Schema for a catalog encoded as JSON(catalog.yaml)]
[source,YAML]
----
...
include::requirements/building-blocks/requirements_class_oas30.adoc[]
