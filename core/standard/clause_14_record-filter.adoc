...
. The link relation is the key for an array of links with a schema similar to that used in the <<query-response,links>> and <<query-response,associations>> sections but omitting the `rel` key.
...
The following example illustrates how to filter links/associations that may
