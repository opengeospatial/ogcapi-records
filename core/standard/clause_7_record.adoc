...
This specification extends the https://github.com/opengeospatial/ogcapi-common/blob/master/core/openapi/schemas/link.json[schema for a link] to include substitution variables who's values are filled in at runtime prior to resolving the link.  The https://schemas.opengis.net/ogcapi/common/part1/1.0/openapi/schemas/link.yaml[link schema] is extended with the addition of three properties named `templated`, `variables` and `varBase`.  

include::requirements/record-core/REQ_templated-link.adoc[]

The `templated`, `variables` and `varBase` properties work together with the `href` property to allow both templated and non-templated aware clients resolve the link.  The `href` property is required and will thus always be available.  If the `templated` property is not specified then the link behaves as a normal link and the value of the `href` member is the URL to the resource.  If, however, the `templated` property is specified, indicating that this is a `templated` link, then templated-aware clients can consider the `href` value as a example URL to the resource (and use that if it is suitable for their needs) OR they can create a resolvable URL to the resource using the substitution variable information retrieved from the `variables` and `varBase` members.  Non-templated aware clients, on the other hand, will simply use the `href` value as the URL to the resource and ignore all the unrecgonized members.

The following example illustrates a link to a legacy OGC Web service.  The example includes both an in-line and referenced (i.e. _variables.json_) dictionary of substitution variables.

[#templated-link-with-variables,reftext=`Associations simple and URI Template Example`]
.Templated link with substitution variables
[source,json]
----
include::../examples/json/templated-link.json[]
----

[#variables-example,reftext=`Dictionary of variables`]
.Referenced dictionary of substitution variables
[source,json]
----
include::../examples/json/variables.json[]
----
