...
This specification defines a new schemas based on the https://github.com/opengeospatial/ogcapi-common/blob/master/core/openapi/schemas/link.json[schema for a link] that includes substitution variables who's values are filled in at runtime prior to resolving the link.  The https://schemas.opengis.net/ogcapi/common/part1/1.0/openapi/schemas/linkTemplated.yaml[templated link schema] defines three properties named `link-template`, `variables` and `var-base`.  The `link-template` property supplies the text of the templated link.  The `variables` and `var-base` properties allow substitution variables to be encoded in-line in the link or externally referenced.

include::requirements/record-core/REQ_templated-link.adoc[]

The following example illustrates a link template to a legacy OGC Web service.  The example includes both an in-line and referenced (i.e. _variables.json_) dictionary of substitution variables.
