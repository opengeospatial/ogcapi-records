...
Association links point to the resource(s) being described by a record. Such links allow binding to a resource once it has be discovered by searching a catalogue.  Depending on how the target resource of a record is represented and how it may be accessed, there may be one or more association links.  For example, if the target resource is an Earth observation imagery file, links may point to the indivdually accessible bands of the image file.
...
Finally, association links are also meant to point to other resources external to the catalogue that are related to the containing record.
