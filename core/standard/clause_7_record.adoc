...
|providers|optional |A list of providers who create or process the resource. |properties.providers
|contacts |optional |A list of contacts qualified by their role in association to the record or resource. |properties.contacts
...
[[sc_providers_and_contacts]]
==== Providers and contacts

A provider is an entity that creates, produces, processes, hosts or publishes
the resource(s) described by the record.

A contact is a related party that can be an administrator, public relations manager, project officer, other contributors or maintainers, etc. of the record or the resource.

include::requirements/record-core/REQ_party.adoc[]
