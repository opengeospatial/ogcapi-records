...
The <<ogc-api-resource-tree,set of OGC API Standards>> define a number of resource endpoints from which can be retrived summary information about sub-resources accessible via the API of the server.  These endpoints are referred to as _local resources catalogs_ in this Standard because they provide metadata about resources offered locally by a deployment of some or all of the <<ogc-api-resource-tree, set of OGC API Standards>>.

[[local-resources-endpoint]]
NOTE: The term _local resources endpoint_ is used to refer the resource path of the _local resources catalog_. +
The term _local resources object_ is used to refer to an instance of the object or resource that can be retrieved from the _local resources endpoint_. +
A _local resources object_ is essentially a <<clause-record-core,record>> describing the resources (e.g. feature collection, process, scenes, etc.) accessible via the server's API.

The `/collections` endpoint is an example of a _local resources endpoint_.  This endpoint provides a list of objects describing the collections of items offered by an OGC API deployment.  Each offered collection (path: `/collections/{collectionId}`) is described by a _local resources object_ containing a small number of properties.  In this respect, the `/collections` endpoint is a catalog of collections (i.e. `/collections/{collectionId}` objects) describing the collections of items offered by the OGC API deployment.
...
2. optionally define or extend the set of query parameters available at the local resources endpoint (e.g. `/collections`) to support advanced searching using a filter language such as CQL2,
...
==== Extending the local resources object

As an example of a local resources endpoint, consider the `/collections` endpoint.  The object that can be retrieved from this endpoint, called a _collection_, is defined in http://docs.opengeospatial.org/is/17-069r3/17-069r3.html#_collections_[OGC API - Features].  The following table cross-walks the information content of the collection with the <<core-properties,properties>> defined for the <<clause-record-core,record-core>> building block:
...
As can be seen, most of the properties of a collection are also record properties.

include::requirements/local-resources-catalog/REQ_mandatory-properties.adoc[]

include::recommendations/local-resources-catalog/PER_additional-properties.adoc[]

include::recommendations/local-resources-catalog/PER_other-properties.adoc[]
