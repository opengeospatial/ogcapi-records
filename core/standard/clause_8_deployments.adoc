...
[NOTE]
==================================================
* _**local_resources_endpoint**_ +
Refers to the path from which a list of accessible local resources can be retrieved.

* _**local_resources_catalog**_ +
Refers to an instance of the resource that can be retrieved from the _local resources endpoint_.

* _**local_resources_item**_ +
Each local resources catalog includes an array member.  The items of that array are objects that contain summary metadata about resources retrievable via the server's API.  These objects are referred to as _**local resources items**_.
==================================================
...
2. Extends the information content of the local resources item to satisfy the requirements of the <<clause-record-core,Record Core>> common component.

3. Optionally extends the local resources endpoint with a set of query parameters that support a basic capability.

4. Optionally extends the local resources endpoint with a set of query parameters that support an advanced searching capability using a filter language such as CQL2.

5. Optionally extends the local resources endpoint with a set of query parameters that support sorting of the server's response.
...
In practice the responsible standard would define how the two sets of properties are merged into the local resources catalog.  In this case it is straight forward since the collections object is very sparse and most catalog properties can be added without issue.  The difference between the `collections` property and the `records` property is handled by requirement <<req_record-collection_records-array-name,/req/record-collection/records-array-name>>.
...
In practice the responsible standard would define how the two sets of properties are merged into the local resources item.  In this case it is straight forward since the collection object and the record share a large number of properties.  Where there is a difference, `extent` versus `geometry` and `time`, can be handled by including both sets of properties in the local resources item and ensuring that their values are the simlar.

==== Examples

===== Collections example

The following JSON Schema fragments illustrate how to extended the https://docs.ogc.org/is/17-069r4/17-069r4.html#_collections_[collections object] (a local resources catalog) and the https://docs.ogc.org/is/17-069r4/17-069r4.html#_collection_[collection object] (a local resources item) to include properties required and recommended by this requirements class.
