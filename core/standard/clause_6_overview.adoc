...
* extend the record schema to enrich its content as necessary
* optionally implement the CQL, Sorting, OpenSearch/ATOM.
* optionally implement the <<clause-cql-filter,CQL>>, <<clause-sorting,sorting>>,<<clause-opensearch,OpenSearch>>/<<record_atom_encoding,ATOM>>.
