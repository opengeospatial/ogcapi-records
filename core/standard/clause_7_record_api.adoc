...
Groups of related records are accesed from a collection (i.e. a catalogue).  The collection provides metadata about the catalogue and includes access metadata for navigating to records from the collection.   The content of the collection is defined in the <<clause-core-record-collection,Record collection clause>> 
...
T.B.D.  Add requirments here for the links section.  Basically, there MUST be at least one link with `rel=items` that points the access/search endpoint of the API. Although, now that I think of it, I believe features includes this requirement.
...
|filter/filter_lang/filter_crs^3^ |Enhanced filtering prameters
2+|NOTES:
