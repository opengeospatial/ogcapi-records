...
^|B |For search terms that are space separated (logical AND) only records whose text fields contain all the search terms specified using the `q` parameter SHALL be in the result set.
|===
