...
sub-resources.  These endpoints are referred to "local resource catalogues" in this specification because they provide metadata about resources offered locally by an OGC API deployment.
...
WARNING: T.B.D.  The rest of this clause was originally a description of extending the `/collections` endpoint but is now being expanded to be any endpoint where it makes sense to add catalogue-like query capability.
